...
resource "null_resource" "file_replacement_api_gw" {
  provisioner "local-exec" {
    command     = <<EOF
sed -i "s,API_GATEWAY_URL,${aws_api_gateway_deployment.apideploy_ba.invoke_url},g" resources/s3/webfiles/build/static/js/main.45b26bb4.js
sed -i "s,API_GATEWAY_URL,${aws_api_gateway_deployment.apideploy_ba.invoke_url},g" resources/s3/webfiles/build/static/js/main.45b26bb4.js.map
sed -i 's/"\/static/"https:\/\/${aws_s3_bucket.bucket_upload.bucket}\.s3\.amazonaws\.com\/build\/static/g' resources/s3/webfiles/build/static/js/main.45b26bb4.js
sed -i 's/n.p+"static/"https:\/\/${aws_s3_bucket.bucket_upload.bucket}\.s3\.amazonaws\.com\/build\/static/g' resources/s3/webfiles/build/static/js/main.45b26bb4.js
EOF
    interpreter = ["/bin/bash", "-c"]
  }
  depends_on = [
...
resource "null_resource" "file_replacement_api_gw_cleanup" {
  provisioner "local-exec" {
    command     = <<EOF
sed -i "s,${aws_api_gateway_deployment.apideploy_ba.invoke_url},API_GATEWAY_URL,g" resources/s3/webfiles/build/static/js/main.45b26bb4.js
sed -i "s,${aws_api_gateway_deployment.apideploy_ba.invoke_url},API_GATEWAY_URL,g" resources/s3/webfiles/build/static/js/main.45b26bb4.js.map
...
data "aws_acm_certificate" "hackicorp" {
  ...
  # Optional: You can add additional filters to narrow down your search
  statuses    = ["ISSUED"]
  most_recent = true
}

data "aws_route53_zone" "selected" {
  name         = "cybersec.practeraco.de"
  private_zone = false
}
...
resource "aws_cloudfront_distribution" "api_distribution" {
  ...
  viewer_certificate {
    ...
    acm_certificate_arn            = data.aws_acm_certificate.hackicorp.arn
    minimum_protocol_version       = "TLSv1.2_2021"
    ssl_support_method             = "sni-only"
  }

  aliases = ["hackicorp.cybersec.practeraco.de"]
}

resource "aws_route53_record" "cdn_cname" {
  zone_id = data.aws_route53_zone.selected.zone_id
  name    = "hackicorp.cybersec.practeraco.de"
  type    = "CNAME"
  ttl     = "300"
